...
namespace NAK.GestureLock
{
    internal class CVRInputModule_XRPatches
    {
        ...
        private static void Postfix_CVRInputModule_XR_Update_Emotes(ref CVRInputModule_XR __instance)
        {
            ...
            {
                //MelonLogger.Msg($"(leftInput && __instance._leftModule.Type == eXRControllerType.Index || __instance._inputManager.oneHanded)");
                return;
            }

            if (rightInput && __instance._rightModule.Type == eXRControllerType.Index || __instance._inputManager.oneHanded)
            {
                //MelonLogger.Msg($"(rightInput && __instance._rightModule.Type == eXRControllerType.Index || __instance._inputManager.oneHanded)");
                return;
            }

            if (leftInput || rightInput)
            {
                //MelonLogger.Msg($"(leftInput || rightInput)");
                _isLocked = !_isLocked;
